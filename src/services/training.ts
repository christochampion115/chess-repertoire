...
// Debug helper — silenced in production
...
function selectTrainingPath(node: RepertoireNode): RepertoireNode[] | null {
  ...
  _dbg('[TRAINING] selectTrainingPath nodeId:', node.id, 'mode:', ts.mode, 'pathsCount:', paths.length);
  if (paths.length === 0) {
    _dbg('[TRAINING] selectTrainingPath => NO PATHS');
    return null;
  }
  ...
  _dbg('[TRAINING] selectTrainingPath scored:', scored.map(s => ({ index: s.index, stopDepth: s.stopDepth, lastId: s.path[s.path.length-1].id, lastSan: s.path[s.path.length-1].san })));

  if (ts.mode === 'vertical') {
    scored.sort((a, b) => (a.stopDepth - b.stopDepth) || (a.index - b.index));
    const result = scored[0]?.path ?? paths[0];
    _dbg('[TRAINING] selectTrainingPath VERTICAL returning path ending at:', result[result.length-1]?.san, 'depth:', scored[0]?.stopDepth);
    return result;
  }
  else if (ts.mode === 'express') {
    scored.sort((a, b) => (b.stopDepth - a.stopDepth) || (a.index - b.index));
    const result = scored[0]?.path ?? paths[0];
    _dbg('[TRAINING] selectTrainingPath EXPRESS returning path ending at:', result[result.length-1]?.san, 'depth:', scored[0]?.stopDepth);
    return result;
  }
  else if (ts.mode === 'horizontal') {
    _dbg('[TRAINING] selectTrainingPath HORIZONTAL returning paths[0] ending at:', paths[0][paths[0].length-1]?.san);
    return paths[0];
  }
  else if (ts.mode === 'survival') {
    _dbg('[TRAINING] selectTrainingPath SURVIVAL returning paths[0] ending at:', paths[0][paths[0].length-1]?.san);
    return paths[0];
  }
  else if (ts.mode === 'randomizer') {
    const idx = Math.floor(Math.random() * paths.length);
    _dbg('[TRAINING] selectTrainingPath RANDOMIZER returning paths[' + idx + '] ending at:', paths[idx][paths[idx].length-1]?.san);
    return paths[idx];
  }
  const result = scored[0]?.path ?? paths[0];
  _dbg('[TRAINING] selectTrainingPath FALLBACK returning:', result[result.length-1]?.san);
  return result;
}
...
export function confirmTrainingStart(): void {
  initNodeMap();
  _dbg('[TRAINING] confirmTrainingStart called', { pendingTrainingMode, pendingTrainingColor: pendingTrainingColor, hasNode: !!pendingTrainingNode });
  ...
  _dbg('[TRAINING] confirmTrainingStart mode:', mode, 'repColor:', repColor, 'nodeId:', node.id, 'nodeFen:', node.fen, 'nodeTurn:', node.turn);
...
function advanceAutoPlay(forcedDelay: number | null = null) {
  const ts = useTrainingStore.getState();
  if (ts.phase !== 'active') { _dbg('[TRAINING] advanceAutoPlay => phase not active'); return; }
  ...
  if (!node) { _dbg('[TRAINING] advanceAutoPlay => node not found for currentId:', currentNodeId); return; }
  _dbg('[TRAINING] advanceAutoPlay nodeId:', node.id, 'san:', node.san, 'turn:', node.turn, 'mode:', ts.mode, 'repColor:', ts.repColor, 'answered:', ts.answered.has(node.id), 'skipped:', ts.skippedByError.has(node.id));

  if (ts.ignoredNoReply.has(node.id)) { _dbg('[TRAINING] advanceAutoPlay => ignoredNoReply, lineComplete'); handleLineComplete(node); return; }
  if (node.children.length === 0) { _dbg('[TRAINING] advanceAutoPlay => leaf, lineComplete'); handleLineComplete(node); return; }

  const nextToPlay = getTrainingNodeTurn(node);

  if (nextToPlay === ts.repColor) {
    _dbg('[TRAINING] advanceAutoPlay => player turn');
    if (!ts.answered.has(node.id) && !ts.skippedByError.has(node.id)) {
      _dbg('[TRAINING] advanceAutoPlay => waiting for player at node:', node.id, node.san);
      ...
      _dbg('[TRAINING] advanceAutoPlay => expectedChildId set to:', expectedId);
      return;
    } else {
      _dbg('[TRAINING] advanceAutoPlay => player turn but already answered/skipped');
    }
  } else {
    _dbg('[TRAINING] advanceAutoPlay => opponent turn, auto-playing');
  }

  const selectedPath = selectTrainingPath(node);
  if (!selectedPath || selectedPath.length === 0) {
    _dbg('[TRAINING] advanceAutoPlay => no path selected, lineComplete');
  ...
  _dbg('[TRAINING] advanceAutoPlay => auto-playing', nextNode.san, 'in', delay, 'ms, steps:', steps);

  if (autoPlayTimer) clearTimeout(autoPlayTimer);
  autoPlayTimer = setTimeout(() => {
    if (useTrainingStore.getState().phase !== 'active') return;
    const curNode = useRepertoireStore.getState().currentNodeId ? nodeMap.get(useRepertoireStore.getState().currentNodeId!) : undefined;
    if (!curNode || curNode.id !== currentId) { _dbg('[TRAINING] advanceAutoPlay timer => stale, curId:', curNode?.id, 'expected:', currentId); return; }
    ...
    _dbg('[TRAINING] advanceAutoPlay timer => navigated to:', nextNode.id, nextNode.san);
    advanceAutoPlay();
  }, delay);
}
...
function handleLineComplete(node: RepertoireNode) {
  const ts = useTrainingStore.getState();
  _dbg('[TRAINING] handleLineComplete nodeId:', node.id, 'san:', node.san, 'mode:', ts.mode, 'fullyExplored(root):', isFullyExplored(ts.root!));
  ts.markVisited(node.id);
  ts.setExpectedChildId(null);

  if (ts.mode === 'express' || ts.mode === 'randomizer') {
    _dbg('[TRAINING] handleLineComplete => direct target mode, showing next');
    showNextTrainingTarget();
    return;
  }

  if (isFullyExplored(ts.root!)) {
    _dbg('[TRAINING] handleLineComplete => root FULLY explored!');
    if (ts.mode === 'survival') {
...
export function checkTrainingMove(moveSan: string): void {
  const ts = useTrainingStore.getState();
  if (ts.phase !== 'active') { _dbg('[TRAINING] checkTrainingMove => phase not active'); return; }

  const currentNodeId = useRepertoireStore.getState().currentNodeId;
  const node = currentNodeId ? nodeMap.get(currentNodeId) : undefined;
  if (!node) { _dbg('[TRAINING] checkTrainingMove => node not found'); return; }
  ...
  _dbg('[TRAINING] checkTrainingMove san:', moveSan, 'nodeId:', node.id, 'nodeSan:', node.san, 'expectedChildId:', expectedChildId, 'isExpected:', isExpectedMove, 'isAlternative:', isAlternativeMove, 'mode:', ts.mode);
